...
- 阶段 13 计划下一步开发。
- 阶段 14-18 已完成范围拆分，待阶段 13 稳定后逐步推进。
...
- 当前主要功能缺口：历史回放仍是事件文本级别，尚未保存每步牌桌状态快照；快速对局仍是 1 名人类玩家 + 3 个 Bot，尚未支持 4 名真人创建和加入同一房间；进行中牌局状态仍以内存为主，服务重启后不能恢复。
...
- 阶段 13 完整牌桌状态快照回放。
- 补齐历史回放数据基础，确保后续多人房间和复杂规则调试时有可复盘能力。
...
4. 阶段 13 优先解决“只能回放事件文本”的缺口，为后续调试多人房间和地方规则提供基础。
5. 地方规则、容器化部署和生产化运维能力在多人房间与恢复能力稳定后继续扩展。
