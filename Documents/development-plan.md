...
- 阶段 14 正在开发，已完成房间大厅、等待室广播、准备、等待/已结束房间退出、牌局中返回大厅处理、开始房间、Bot 补位、真实牌局创建、多人玩家视角同步、牌局结束后房间状态同步和断线宽限基础。
- 阶段 15-18 已完成范围拆分，待阶段 14 稳定后逐步推进。
...
- 阶段 14 已完成房间大厅、等待室广播、准备、等待/已结束房间退出和多人牌局同步基础：新增内存房间大厅服务、创建房间/加入房间/查询当前房间/退出当前房间 HTTP API，玩家大厅启用创建、加入和退出房间入口，展示 4 个座位状态，通过 Socket.IO 广播同房间座位变化，支持玩家准备/取消准备，等待房间内普通玩家可退出并清空座位，房主退出时转移房主，最后一名玩家退出时清理空房；房主可在所有真人玩家准备后开始房间并用 Bot 补齐空座；房间开始后会创建真实 Mahjong 牌局，多个真人玩家可按各自座位获得独立 `PlayerView`，服务端按登录用户座位校验动作，房间内 Socket 连接按用户分别推送视角；牌桌中返回大厅或退出登录会先确认并触发服务端离局处理，单人快速局直接流局结束，多人局由 Bot 接手离开玩家座位继续推进，如果最后一名真人离开则流局结束；牌局结束后大厅房间会标记为已结束并广播，玩家回到大厅可以看到房间结束状态，并可退出已结束房间、直接加入其他等待房间或用新房间号再开一局；服务端会阻止玩家在等待/进行中房间里重复创建新房间，避免房间归属和牌局归属分叉。
- 当前主要功能缺口：房间结束后回到等待室并用原成员再开一局等生命周期规则尚未完善；进行中牌局状态仍以内存为主，服务重启后不能恢复。
...
将当前快速对局从单人加 Bot 模式扩展为真实房间模式，支持玩家创建房间、输入房间号加入、选择座位、准备开局，并在人数不足时由 Bot 补位。

阶段状态：进行中，已完成房间大厅、等待室广播、准备、等待/已结束房间退出、牌局中返回大厅处理、开始房间、Bot 补位、真实牌局创建、多人玩家视角同步、牌局结束后房间状态同步和 20 秒断线宽限。
...
- 阶段 14 真实多人房间。
- 支持创建房间、加入房间、退出等待/已结束房间、座位、准备、Bot 补位、多人牌局同步和断线宽限，继续补齐原房间再开局生命周期。
- 保持现有简单规则、快速对局和历史页面稳定。

第二优先级：

- 阶段 15 进行中牌局恢复和房间生命周期。
- 阶段 16 规则配置化增强。
- 阶段 17 管理员对局后台。
...
5. 阶段 14 已完成进行中退出和断线宽限基础，下一步优先补齐牌局结束回房和原房间再开局；地方规则、容器化部署和生产化运维能力在多人房间与恢复能力稳定后继续扩展。
