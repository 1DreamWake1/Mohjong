...
- 阶段 15 已完成进行中牌局恢复和房间生命周期管理，阶段 16 已于 2026-07-31 完成规则配置化增强，阶段 17 管理员对局后台已完成，阶段 18A 和 18B 已于 2026-08-03 完成。
...
- 阶段 15 已完成恢复快照、服务启动恢复、异常结束诊断，以及等待和已结束房间的定时清理。
- 阶段 16 已完成规则预设、牌种、动作、响应优先级、胡牌结构、番型开关、番值、计分模式、封顶番数、起胡番数、配置校验和副露计分边界，并完成四川麻将独立阶段设计。
- 阶段 17 已完成管理员对局后台：全量历史记录、组合筛选、结算和快照回放、活跃房间、玩家连接状态与持久化失败诊断均已落地。
- 当前主要功能缺口：尚未完成数据卷备份恢复、升级回滚、生产安全基线和自动化部署验收；四川麻将完整玩法按独立后续阶段实现。
...
阶段状态：已于 2026-07-31 完成。规则配置边界、计分策略、合法性校验、恢复兼容和四川麻将后续拆分均已落地。
...
- `scoring.mode` 已接入实际计分：标准模式按底分加每番分值线性计算，四川模式按底分乘 `2^番数` 计算；`fanLimit` 可配置封顶番数，旧快照默认不封顶。
- 核心包统一校验规则名称、版本、响应优先级、番值和计分数值；新对局拒绝非法规则，恢复流程将非法规则快照标记为不可恢复。
- `scoring.minimumFan` 配置起胡番数；自摸、点炮、可见合法动作和 Bot 决策统一遵守该门槛，旧快照默认 0 番起胡。
- 副露玩家的胡牌结构继续按剩余暗手判断，番型识别改为读取暗手和公开组合的完整牌面，避免漏算对对胡或误判清一色、混一色、断幺九。
- `canHu` 显式接收副露数量并校验暗手必须为 `14 - 3 × 副露数` 张，避免损坏状态中的暗手与公开组合数量不一致时被误判为胡牌。
...
- 已完成四川麻将能力边界和阶段拆分设计，覆盖换三张、定缺、血战到底、刮风下雨、查叫、花猪和退税；完整实现按独立阶段推进，详见 [sichuan-rules-design.md](./sichuan-rules-design.md)。
...
将本地开发验证推进到可重复部署、可备份恢复、可监控和可安全退出的单实例长期运行形态。生产部署以 Docker Compose 为主，由反向代理统一提供 Web、HTTP API 和 Socket.IO 入口；补齐容器构建、生产配置、SQLite 数据卷、健康检查、日志、备份恢复、升级回滚和自动化验收。

阶段状态：进行中（18A 和 18B 已于 2026-08-03 完成，下一步开发 18C SQLite 备份、恢复和升级回滚）。

架构边界：

- 本阶段只支持一个 Server 实例。房间索引、Socket 连接、Bot 调度和倒计时仍由单进程管理，不允许通过增加 Server 副本实现水平扩容。
- SQLite 继续作为本阶段生产数据存储，数据库和备份目录使用持久化数据卷。
- Redis、Socket.IO Redis Adapter、分布式锁和 PostgreSQL 属于未来多实例阶段，不作为阶段 18 验收阻塞项。
- 不以 PM2 或 systemd 直跑 Node.js 作为主要部署路线。

### 21.2 实施批次

#### 21.2.1 阶段 18A：容器化和统一入口

- 为 Server 增加多阶段 Dockerfile，只把生产运行所需依赖、Prisma Client、迁移文件和构建产物放入最终镜像。
- 为 Web 增加多阶段 Dockerfile，使用 Node.js 构建静态资源，由 Nginx 或 Caddy 提供生产静态文件。
- 增加 `.dockerignore`，排除 `.git`、`node_modules`、`dist`、本地数据库、日志、测试缓存和真实环境文件。
- 增加 `compose.yaml`，编排 Web、Server 和 SQLite 数据卷，并配置明确的服务依赖、重启策略和端口。
- 反向代理统一处理 `/`、HTTP API 和 `/socket.io`，前端生产环境默认使用同源地址，不再依赖固定的 `:3000` 后端端口。
- Server 启动时先执行 `prisma migrate deploy`，迁移成功后再接受请求；迁移失败必须使容器启动失败。
- 镜像使用非 root 用户运行，并固定 Node.js、pnpm 和基础镜像版本。

#### 21.2.2 阶段 18B：健康检查和进程生命周期

完成状态：已于 2026-08-03 完成。

- 保留轻量存活检查，并新增数据库可访问、恢复初始化完成的 readiness 检查。
- Compose 健康检查必须调用真实 HTTP 健康端点，Web 对外服务只依赖健康的 Server。
- 处理 `SIGTERM` 和 `SIGINT`：停止接收新请求、关闭 Socket.IO、取消房间清理和操作计时任务、等待同房间持久化队列完成、断开 Prisma 后退出。
- 为优雅退出设置最大等待时间，超时后输出结构化错误并以非零状态退出。
- 避免服务关闭时仍创建新的 Bot 操作、超时托管或数据库写入。
- 增加启动、恢复完成、健康状态变化和退出过程的结构化日志。

#### 21.2.3 阶段 18C：SQLite 备份、恢复和升级回滚

- 数据库文件与备份文件分别挂载到明确的持久化目录，禁止写入容器临时文件系统。
- 提供一致性备份脚本，运行中使用 SQLite `.backup` 或等价在线备份能力，不直接复制正在写入的数据库文件。
- 备份文件包含时间、应用版本和 schema 迁移信息，并支持保留周期和过期清理。
- 提供恢复脚本：停止写入、保留当前数据库、恢复指定备份、执行一致性检查和迁移校验，再启动服务。
- 提供升级流程：先备份、拉取新镜像、执行迁移、健康检查；失败时按文档回退镜像和数据库。
- 增加备份恢复演练，验证用户、历史对局、事件和恢复快照在新数据卷中可读取。
- 文档说明何时需要从 SQLite 迁移 PostgreSQL，例如多实例、持续高写入或更严格的在线备份要求。

#### 21.2.4 阶段 18D：生产配置和安全基线

- 区分开发与生产配置；生产环境缺少 `AUTH_TOKEN_SECRET`、数据库路径或必要代理配置时拒绝启动。
- `.env.example` 只保留无敏感信息的占位值，真实密钥、管理员密码、数据库和备份文件不得提交。
- 限制 CORS 为配置的可信来源；同源部署默认不开放任意来源。
- 登录接口增加速率限制和失败退避，管理员初始化密码提高最低强度要求。
- 配置请求体大小、Socket 连接频率和游戏动作频率限制，避免单个客户端耗尽资源。
- 为 Web 增加基础安全响应头和 CSP；继续使用 `localStorage` Token 时，文档明确 XSS 风险和后续 HttpOnly Cookie 迁移方向。
- 评估将自有 JWT 实现迁移到成熟库；若本阶段不迁移，至少增加密钥长度、过期、篡改和异常输入测试。
- 管理员诊断继续提供进程内最近错误，同时确保关键持久化失败写入结构化日志，避免重启后完全失去排查线索。

#### 21.2.5 阶段 18E：自动化验收和运行手册

- 增加 CI，至少执行 `pnpm install --frozen-lockfile`、Prisma Client 生成、类型检查、Lint、格式检查、全量测试和生产构建。
- CI 构建 Docker 镜像并执行 Compose 冒烟测试，验证迁移、健康检查、Web 页面、登录 API 和 Socket.IO 连接。
- 增加 Playwright 关键流程测试：登录、创建房间、两名真人准备开局、操作倒计时、断线重连、牌局结束和历史记录查询。
- 增加部署运行手册，覆盖环境准备、配置、首次启动、管理员初始化、迁移、升级、日志查看、备份、恢复、回滚和常见故障。
- 记录端口、数据卷、健康检查、日志字段和反向代理路径，确保新环境无需阅读源码即可部署。
- 保持 `pnpm dev` 面向本地开发，Docker/Compose 命令面向生产式运行，避免脚本相互隐式依赖。

### 21.3 验收标准

- 全新 Linux 环境可以按运行手册完成配置、镜像构建、数据库迁移、管理员初始化和启动。
- Docker Compose 一条命令启动完整服务，Web、API 和 Socket.IO 通过同一对外入口工作。
- Server 和 Web 镜像不包含本地数据库、真实环境文件、开发依赖或不必要源码，并以非 root 用户运行。
- 存活检查与 readiness 检查可区分进程存活和数据库不可用；Compose 能正确识别服务健康状态。
- 收到 `SIGTERM` 后服务停止接收新请求，完成或明确终止持久化队列，并正常断开 Prisma。
- SQLite 数据保存在持久化卷中；在线备份、指定备份恢复、一致性检查和保留清理均有可执行命令。
- 完成一次升级失败回滚和一次空数据卷恢复演练，恢复后的账号、历史记录和对局事件可正常查询。
- 生产配置缺少必要密钥时拒绝启动，CORS、登录和 Socket 入口具备基础访问限制。
- CI 可重复完成依赖安装、Prisma 生成、静态检查、213 项以上测试、构建、镜像构建和 Compose 冒烟测试。
- Playwright 覆盖至少一条真实多人对局关键路径。
- Docker 部署不要求提交真实密钥，备份、数据库和日志产物均不进入 Git。
- `pnpm typecheck`、`pnpm lint`、`pnpm test`、`pnpm build` 通过。

### 21.4 非本阶段范围

- Server 多实例、房间跨进程迁移和无状态 Socket 扩容。
- Redis、消息队列、分布式锁和 PostgreSQL 正式迁移。
- Kubernetes、自动弹性扩缩和跨地域高可用。
- 四川麻将完整玩法、牌桌素材重做和大规模前端视觉改版。
- 完整安全审计或互联网高并发承诺；本阶段只建立单实例长期运行所需的安全基线。
...
- 依次完成 18A 容器化、18B 生命周期、18C 备份恢复、18D 安全基线和 18E 自动化验收。
- 保持本地开发脚本、数据库迁移和生产部署边界清晰。

第二优先级：

- UI 贴图升级、更多牌桌体验细节和移动端交互优化。
- 按 [sichuan-rules-design.md](./sichuan-rules-design.md) 规划四川麻将独立开发阶段，不在未完成闭环时暴露玩法入口。
...
6. 阶段 15 已完成进行中牌局持久化、服务重启恢复、异常记录和房间清理策略。
7. 阶段 17 已完成管理员历史对局、活跃房间、连接状态和持久化错误诊断后台。
8. 阶段 16 已完成规则配置化、计分策略、配置校验、副露胡牌边界和四川麻将后续阶段设计，下一步推进阶段 18。
